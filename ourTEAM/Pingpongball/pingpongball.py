...
# 무기 만들기
weapon = pygame.image.load(os.path.join(image_path, "beam1.png"))
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로
               character = pygame.image.load(os.path.join(image_path, "ch1lr.png"))
               character_to_x -= character_speed
            elif event.key == pygame.K_RIGHT: # 캐릭터를 오른쪽으로  
                character = pygame.image.load(os.path.join(image_path, "ch1rr.png"))
                character_to_x += character_speed
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        if character_rect.colliderect(ball_rect):
            character_y_pos += 20
            character = pygame.image.load(os.path.join(image_path, "chfr.png"))
            running = False
            break
