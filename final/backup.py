...
default_font = pygame.font.Font('YiSunRegular.ttf', 28)
...
warning_img1 = pygame.image.load('warning1.gif')
...
def game_loop():
    ...
    # 밑에서 변수 t가 if 문의 타이머 조건인 5000 (5초)를 함부로 넘지 못하도록 엄청 큰 값을 세팅
    ticks = 99999999999
        ...
        if paused:
            ...
            if random.randint(1, occur_prob) == 1:
                ...
                if random.randint(1, occur_prob * 10) == 1:
                    ...
                    warps.add(warp)
                if random.randint(1, occur_prob * 1) == 1:  # 워프 발생주기와 똑같게 블랙홀 생성
                    if blackhole_count == 0:
                        ...
                        # sleep(2)  # 2초 슬립주고 warning 띄우고 사이렌 울리기 구현하기!!!!!!!
            ...
            if blackhole_count == 1 and t < 5000:  # 블랙홀이 생성되어있고, 아직 5초가 지나지 않아서 살아있다면
                for rock in rocks:  # 그룹화 되어있는 rocks에서 모든 rock에 개별 접근한다
                    ...
                    if coordinate_rock_x == 0 and coordinate_rock_y != 0:
                        ...
                        if coordinate_rock_x > 0 and coordinate_rock_y > 0:
                            ...
                            if t >= 4500 and t < 5000:  # timer가 4.5초에서 5초 사이일 때, 각각의 rock들을 이전에 가지고 있던 속도로 초기화
                ...
                for scurge in scurges:  # 그룹화 되어있는 scurges에서 모든 scurge에 개별 접근한다
                    ...
                    if coordinate_scurge_x == 0 and coordinate_scurge_y != 0:
                        ...
                        if coordinate_scurge_x > 0 and coordinate_scurge_y > 0:
                            ...
                            if t >= 4500 and t < 5000:  # timer가 4.5초에서 5초 사이일 때, 각각의 scurge들을 이전에 가지고 있던 속도로 초기화
            ...
            if t > 5000:  # 그 시간 차이가 5초 이상이되면
                # 변수 t가 if 문의 타이머 조건인 5000 (5초)를 함부로 넘지 못하도록 엄청 큰 값을 세팅
                blackhole_count -= 1
            ...
            draw_text('점수: {}'.format(score),
                      default_font, screen, 80, 20, YELLOW)
            draw_text('워프: {}'.format(warp_count),
                      default_font, screen, 700, 20, BLUE)
            rocks.update()
            warps.update()
            scurges.update()
            ...
            for event in pygame.event.get():
                if event.type == pygame.MOUSEMOTION:
                    ...
                    elif mouse_pos[0] >= WINDOW_WIDTH - 10:  # 마우스의 x좌표가 우측 창 기준 10에 근접했을 때
